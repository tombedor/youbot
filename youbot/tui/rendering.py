...
from youbot.core.models import CodingAgentActivitySnapshot, RepoRecord
...
def build_activity_panel(snapshot: CodingAgentActivitySnapshot | None) -> Panel:
    if snapshot is None:
        return Panel(
...
def build_repo_panel(
    active_repo_id: str | None, repo: RepoRecord | None, cache: dict[str, object]
) -> RenderableType:
...
def repo_header(active_repo_id: str | None, repo: RepoRecord | None) -> tuple[str, str]:
    if active_repo_id is None:
