...
class RepoViewBuilder:
    ...
    def _render_successful_overview(
        self,
        repo_id: str,
        result: ExecutionResult,
        spec: OverviewSectionSpec,
        ...
        if spec.render_mode == "json" and result.parsed_payload is not None:
            ...
            if isinstance(payload, dict):
                return self._render_json_overview(repo_id, payload, spec)
    ...
    def _render_job_search_pipeline(self, payload: dict[str, Any]) -> RenderableType:
        active_rows = []
        closed_rows = 0
    ...
    def _render_job_search_openings(self, payload: dict[str, Any]) -> RenderableType:
        summary = Text()
    ...
    def _render_checklist(self, items: list[dict[str, Any]], *, max_items: int) -> Text:
        lines = [f"- {item.get('text', '')}" for item in items[:max_items]]
        if len(items) > max_items:
            lines.append(f"... {len(items) - max_items} more")
        return Text("\n".join(lines))

    def _render_life_admin_digest(self, payload: dict[str, Any]) -> RenderableType:
    ...
    def _render_task_list(self, tasks: list[dict[str, Any]], *, max_items: int) -> Table:
        table = Table(expand=True, box=None, show_header=True)
    ...
    def _render_trader_findings(self, text: str) -> Table:
        strategies: list[str] = []
    ...
    def _render_trader_program(self, text: str, max_lines: int) -> RenderableType:
        goal = ""
